...
class TestHrAttendanceIPCheck(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.group_attendance_user = cls.env.ref('hr_attendance.group_hr_attendance_user')
        cls.group_attendance_manager = cls.env.ref('hr_attendance.group_hr_attendance_manager')
        ...
        cls.hr_user = cls.env['res.users'].with_context(no_reset_password=True).create({
            ...
            'groups_id': [(6, 0, [cls.group_hr_user.id, cls.group_attendance_user.id])]
        })
        ...
        cls.hr_manager = cls.env['res.users'].with_context(no_reset_password=True).create({
            ...
            'groups_id': [(6, 0, [cls.group_hr_manager.id, cls.group_attendance_manager.id])]
        })
        ...
        cls.work_location = cls.env['hr.work.location'].sudo().create({
            'name': 'Test Location',
            'check_ip': False,
        ...
        cls.env['ir.config_parameter'].sudo().set_param('hr_attendance.ip_check_enabled', 'True')
    ...
    def test_04_attendance_allowed_ip(self):
        ...
        with patch('odoo.addons.hr_attendance_location_ip_check.models.hr_employee.HrEmployee._get_remote_ip',
    ...
    def test_07_multiple_cidrs(self):
        ...
        with self.with_user('hr_manager@test.com'):
            ...
            second_cidr = self.env['hr.work.location.cidr'].create({
                'work_location_id': self.work_location.id,
    ...
    def test_10_cidr_sequence(self):
        ...
        with self.with_user('hr_manager@test.com'):
            specific_cidr = self.env['hr.work.location.cidr'].create({
                'work_location_id': self.work_location.id,
    ...
    def test_13_config_changes(self):
        ...
        # Re-enable IP check
        self.env['ir.config_parameter'].sudo().set_param('hr_attendance.ip_check_enabled', 'True')
    ...
    def test_14_bypass_features(self):
        ...
        # Test with manager switching bypass
        with self.with_user('hr_manager@test.com'):
            # Enable bypass
    ...
    def test_15_multi_user_scenarios(self):
        ...
        # HR User tries to create CIDR (should fail)
        with self.with_user('hr_user@test.com'):
            with self.assertRaises(AccessError):
                self.env['hr.work.location.cidr'].create({
                    'work_location_id': self.work_location.id,
        ...
        # HR Manager creates CIDR (should succeed)
        with self.with_user('hr_manager@test.com'):
            cidr = self.env['hr.work.location.cidr'].create({
                'work_location_id': self.work_location.id,
